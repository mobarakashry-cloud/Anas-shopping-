...
export default function AdminDashboardClient({ user }: { user?: any }) {
  ...
  const aiRecommendations = [
    { type: 'marketing', title: 'Promote Electronics Category', description: 'Electronics category shows 25% higher engagement this week', priority: 'high' },
    { type: 'inventory', title: 'Restock Popular Items', description: '5 products are running low on inventory', priority: 'medium' },
    { type: 'pricing', title: 'Adjust Pricing Strategy', description: 'Competitive analysis suggests price optimization for Fashion category', priority: 'low' },
  ]
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                {user ? (<span>Signed in as <strong>{user.username || user.id}</strong></span>) : <span>Not signed in</span>}
              </div>
            </div>
            <Link href="/">
      ...
      <main className="px-4 py-6 space-y-6">
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card className="p-4 space-y-4">
              <div className="flex items-center justify-between"><h2 className="font-semibold">Recent Orders</h2><Button variant="ghost" size="sm">View All</Button></div>
              <div className="space-y-2">
                {recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                    <div className="space-y-1"><p className="font-medium text-sm">{order.id}</p><p className="text-xs text-muted-foreground">{order.customer}</p></div>
                    <div className="text-right space-y-1"><p className="font-semibold text-sm">{order.amount}</p><Badge variant={order.status === 'completed' ? 'default' : 'secondary'} className="text-xs">{order.status}</Badge></div>
                  </div>
                ))}
              </div>
            </Card>
          ...
          {/* Additional tabs omitted for brevity - original content kept in repo */}
        </Tabs>
      </main>
    </div>
  )
